...
func main() {
	fmt.Println("🧪 Testing delegation functionality...")

	// Load config
	cfg, err := config.Load("smart-config.yaml")
	if err != nil {
		log.Fatal("❌ Failed to load config:", err)
	}

	// Build agent using the same function as the main app
	agent, err := buildAgent(cfg)
	if err != nil {
		log.Fatal("❌ Failed to build agent:", err)
	}

	fmt.Printf("✅ Agent built with %d tools\n", len(agent.Tools))

	// Check if agent tool exists
	if agentTool, exists := agent.Tools["agent"]; exists {
		fmt.Printf("✅ Agent tool found: %s\n", agentTool.Description())
	} else {
		log.Fatal("❌ Agent tool not found in registry")
	}
	...
	// Verify team context
	if teamFromCtx, ok := team.FromContext(ctx); ok {
		fmt.Printf("✅ Team context verified\n")
		_ = teamFromCtx // suppress unused warning
	} else {
		log.Fatal("❌ Team context not set correctly")
	}

	// Test 1: Direct team call
	fmt.Println("\n📞 Test 1: Direct team call...")
	result, err := tm.Call(ctx, "coder", "Hello, create a simple test file")
	if err != nil {
		fmt.Printf("❌ Direct team call failed: %v\n", err)
	} else {
		fmt.Printf("✅ Direct team call succeeded: %s\n", result[:min(100, len(result))]+"...")
	}

	// Test 2: Agent tool call
	fmt.Println("\n🔧 Test 2: Agent tool call...")
	if agentTool, exists := agent.Tools["agent"]; exists {
		args := map[string]any{
			"agent": "coder",
			"input": "Hello, this is a test delegation",
		}
	...
	// Test 3: Full agent run with delegation prompt
	fmt.Println("\n🚀 Test 3: Full agent run with delegation...")
	fullResult, err := agent.Run(ctx, "Please delegate to a coder agent to create a hello world program")
	if err != nil {
		fmt.Printf("❌ Full agent run failed: %v\n", err)
	} else {
		fmt.Printf("✅ Full agent run succeeded: %s\n", fullResult[:min(200, len(fullResult))]+"...")
	}

	fmt.Println("\n🎉 Delegation testing completed!")
}
