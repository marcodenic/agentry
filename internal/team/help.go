...
func (t *Team) checkWorkCompleted(agentID, task string) bool {
    ...
    for _, keyword := range fileCreationKeywords {
        if strings.Contains(taskLower, keyword) { hasFileWork = true; break }
    }
    if !hasFileWork { return false }
    workDir, err := os.Getwd(); if err != nil { return false }
    recentThreshold := time.Now().Add(-5 * time.Minute)
    hasRecentFiles := false
    filepath.Walk(workDir, func(path string, info os.FileInfo, err error) error {
        if err != nil { return nil }
        if strings.Contains(path, "/.git/") || strings.Contains(path, "/.") { return nil }
