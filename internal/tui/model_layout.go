...
func (m Model) handleWindowResize(msg tea.WindowSizeMsg) (Model, tea.Cmd) {
	...
	m.view.Input.SetWidth(msg.Width - 1)
	...
	if info, ok := m.infos[m.active]; ok {
		if !m.view.Chat.ShowInitialLogo() && info.History != "" {
			if m.view.Chat.LastWidth() == 0 || chatWidth != m.view.Chat.LastWidth() {
				...
				m.view.Chat.Main.SetContent(reformattedHistory)
				m.view.Chat.SetLastWidth(chatWidth)
			} else {
		...
		m.view.Chat.Main.GotoBottom() // Ensure we're at the bottom after resize
	}
	return m, nil
}
